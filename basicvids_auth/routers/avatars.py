...
from basicvids_auth.decorators.auth import authenticated
from basicvids_auth.models.avatars import AvatarDeleteResponse, AvatarPublic
from basicvids_auth.rate_limit import client_identifier, enforce_rate_limit
from basicvids_auth.schemas import get_session
from basicvids_auth.schemas.users import Avatar, User, utc_now
from basicvids_auth.settings import settings
from basicvids_auth.storage import get_storage
from basicvids_auth.storage.base import StorageBackend
...
async def create_registration_avatar(
    ...
    return await save_avatar(user_id, avatar, session, storage, replace_existing=False)
...
async def set_current_user_avatar(
    ...
    await enforce_rate_limit("avatar_upload_user", f"user:{request.state.user.id}", 20, 3600)
    return await save_avatar(request.state.user.id, avatar, session, storage, replace_existing=True)
...
async def delete_current_user_avatar(
    ...
    avatar = session.get(Avatar, request.state.user.id)
    if not avatar:
        raise HTTPException(status_code=404, detail="Avatar not found")
