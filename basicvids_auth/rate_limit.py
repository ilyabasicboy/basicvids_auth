...
async def enforce_rate_limit(name: str, identifier: str, limit: int, window_seconds: int) -> None:
    ...
    key = f"rate_limit:auth:{name}:{identifier}"
...
def rate_limit_ip(name: str, limit: int, window_seconds: int) -> Callable:
    async def dependency(request: Request) -> None:
        await enforce_rate_limit(name, client_identifier(request), limit, window_seconds)
